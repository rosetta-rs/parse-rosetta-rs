...
impl<'a> Parser<'a> {
    ...
    fn r#value(&mut self, diags: &mut Vec<Diagnostic>) {
        match self.current {
            Token::LBrace => {
                self.r#object(diags);
            }
            ...
            | Token::String
            | Token::True => {
                self.r#literal(diags);
            }
            _ => {
                self.error(diags, err![self.span(), "false",
                            "{",
                            "[",
                            "null",
                            "<number>",
    ...
    fn r#object(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        ...
        self.close(m, Rule::Object, diags);
    }
    fn r#member(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        expect!(String, "<string>", self, diags);
        expect!(Colon, ":", self, diags);
        self.r#value(diags);
        self.close(m, Rule::Member, diags);
    }
    fn r#array(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        ...
        self.close(m, Rule::Array, diags);
    }
    fn r#literal(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        match self.current {
            Token::String => {
                expect!(String, "<string>", self, diags);
        ...
        self.close(m, Rule::Literal, diags);
    }
}
