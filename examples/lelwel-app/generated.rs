...
impl<'a> Parser<'a> {
    ...
    fn rule_value(&mut self, diags: &mut Vec<Diagnostic>) {
        match self.current {
            Token::LBrace => {
            ...
            _ => {
                self.error(diags, err![self, "false",
                            "{",
                            "[",
                            "null",
                            "<number>",
    ...
    fn rule_object(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        ...
        let closed = self.cst.close(m, Rule::Object);
        self.create_node_object(NodeRef(closed.0), diags);

    }
    fn rule_member(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        expect!(String, "<string>", self, diags);
        expect!(Colon, ":", self, diags);
        self.rule_value(diags);
        let closed = self.cst.close(m, Rule::Member);
        self.create_node_member(NodeRef(closed.0), diags);

    }
    fn rule_array(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        ...
        let closed = self.cst.close(m, Rule::Array);
        self.create_node_array(NodeRef(closed.0), diags);

    }
    fn rule_literal(&mut self, diags: &mut Vec<Diagnostic>) {
        let m = self.cst.open();
        match self.current {
            Token::String => {
                expect!(String, "<string>", self, diags);
            ...
            Token::Number => {
                expect!(Number, "<number>", self, diags);
            }
            Token::True => {
        ...
        let closed = self.cst.close(m, Rule::Literal);
...
trait ParserCallbacks {
    ...
    fn create_node_array(&mut self, _node_ref: NodeRef, _diags: &mut Vec<Diagnostic>) {}
    ...
    fn create_node_literal(&mut self, _node_ref: NodeRef, _diags: &mut Vec<Diagnostic>) {}
    /// Called when `member` node is created.
    fn create_node_member(&mut self, _node_ref: NodeRef, _diags: &mut Vec<Diagnostic>) {}
    /// Called when `object` node is created.
    fn create_node_object(&mut self, _node_ref: NodeRef, _diags: &mut Vec<Diagnostic>) {}
    /// Called when `value` node is created.
    fn create_node_value(&mut self, _node_ref: NodeRef, _diags: &mut Vec<Diagnostic>) {}
